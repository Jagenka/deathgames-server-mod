        ...
        ifServerLoaded { server ->
            captureProgress.forEach { (spawn, progress) ->
                ...
                val particles = angles.map {
                    ...
                    return@map listOf(
                        spawn.coordinates.toVec3().add(Vec3d(offsetX, height, offsetZ)),
//                        spawn.coordinates.toVec3d().add(Vec3d(offsetX, 0.75 + Math.random() / 2.0, offsetZ)),
                ...
                repeat((captureDistance.pow(2.0) * 1.0).toInt()) {
                    ...
                    particles.add(spawn.coordinates.toVec3().add(Vec3d(offsetX, 0.1, offsetZ)))
                }
                ...
                PlayerManager.getOnlinePlayers().forEach inner@{ player ->
                    ...
                    if (player.pos.subtract(spawn.coordinates.toVec3()).lengthSquared() > VISIBILITY_RANGE.pow(2.0))
                    {
                        return@inner
                    }
    ...
    fun renderOrb(captureProgress: Map<DGSpawn, Int>)
    {
        ...
        ifServerLoaded { server ->
            captureProgress.forEach { (spawn, progress) ->
                ...
                val orb = spawn.coordinates.toVec3().add(Vec3d(0.0, 6.0, 0.0))
                ...
                val beamOrigin = spawn.coordinates.toVec3()
                    .add(Vec3d(1.0, 0.0, 0.0).multiply(Config.spawns.platformRadius.toDouble()))
