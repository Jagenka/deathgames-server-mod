...
data class BlockPos(val x: Int, val y: Int, val z: Int)
{
    ...
    infix fun distanceTo(vec3d: Vec3): Double =
        sqrt((this.x.toCenter() - vec3d.x).pow(2) + (this.y - vec3d.y).pow(2) + (this.z.toCenter() - vec3d.z).pow(2))

    fun hasInRange(pos: Vec3, range: Double): Boolean = this distanceTo pos <= range
    ...
    companion object
    {
        fun from(vec3d: Vec3) = from(vec3d.x, vec3d.y, vec3d.z)
